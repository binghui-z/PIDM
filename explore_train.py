...
# os.environ["WANDB_API_KEY"] = "f8cbc65964cc555bbeaf82d034a1d101513fb436" 
os.environ["WANDB_MODE"] = "offline"  # directly modify the type of wandb
...
def train(conf, loader, val_loader, model, ema, diffusion, betas, optimizer, scheduler, guidance_prob, cond_scale, device, wandb):
    ...
    for epoch in range(300):
        ...
        if (epoch)%args.save_wandb_images_every_epochs==0:
            ...
            grid = torch.cat([(val_img+1)/2, val_pose[:,:3], (samples+1)/2], -1)
            ...
            if is_main_process():
                ...
                grid = torchvision.utils.make_grid(tensor = grid)
                wandb.log({'samples':wandb.Image(grid)})
                save_image(grid, conf.training.ckpt_path + str(f'/sample_epoch-{epoch}.png'))
...
if __name__ == "__main__":
    ...
    DiffConf.ckpt = r"checkpoints\2023-06-28_00-56-12\last.pt"
